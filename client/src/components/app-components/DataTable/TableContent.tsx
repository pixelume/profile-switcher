...
import { iterationsColumns } from "./iterationsColumns";
...
export function TableContent({
  ...
  const [createEntry] = useMutation(CREATE_ITERATION, {
    refetchQueries: [{ query: GET_PAGE, variables: { name: "page1" } }],
  });

  const table = useReactTable({
    data: filteredData,
    columns: iterationsColumns,
        ...
        <div className="flex items-center justify-between gap-x-4">
          ...
          <div className="flex items-center gap-x-10">
            <SearchInput
              ...
              className="max-w-xs"
            />
            <RemoveComponentButton id={componentId} />
          </div>
        </div>
      </CardHeader>
      <CardContent>
          ...
          <Table>
            ...
            <TableBody>
              {table.getRowModel().rows?.length ? (
                ...
                <TableRow>
                  <TableCell
                    colSpan={iterationsColumns.length}
